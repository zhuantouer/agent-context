...
- `HANDOFF.md`: current task, next action, blockers, latest validation, user instructions; clear stale task state.
- `ARCHITECTURE.md`: map, stack, entry points, boundaries, data flow, terms; on structural change, update the module map and its `verified against: <commit>` line. Only claim re-verification for paths you actually re-read.
